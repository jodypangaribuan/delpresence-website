...
func ValidateCampusToken(token string) (*CampusTokenClaims, error) {
	...
	if err := json.Unmarshal(jsonPayload, &uidClaims); err == nil && uidClaims.UID != nil {
		...
		// or infer these values from the request context
		return &CampusTokenClaims{
			UserID:   userID,
			Username: uidClaims.Username,
			Role:     "Dosen", // Default to Dosen role for lecturer endpoints
		}, nil
	}
	...
	if err := json.Unmarshal(jsonPayload, &genericMap); err == nil {
		...
		userID := uint(0)
		username := ""
		role := "Dosen" // Default to Dosen for lecturer endpoints
...
func CampusAuthMiddleware() gin.HandlerFunc {
	return func(c *gin.Context) {
		...
		log.Printf("Set context for request: userID=%v, username=%s, role=%s", campusClaims.UserID, username, role)
