...
// UpdateAcademicYearStatus updates just the status of an academic year
func (h *AcademicYearHandler) UpdateAcademicYearStatus(c *gin.Context) {
	idStr := c.Param("id")
	id, err := strconv.ParseUint(idStr, 10, 64)
	if err != nil {
		c.JSON(http.StatusBadRequest, gin.H{"error": "Invalid ID format"})
	...
	// Update the status
	academicYear.IsActive = statusUpdate.IsActive

	// If activating, use service method that handles deactivating others
	if statusUpdate.IsActive {
		if err := h.service.ActivateAcademicYear(uint(id)); err != nil {
			c.JSON(http.StatusInternalServerError, gin.H{"error": err.Error()})
			return
		}
	} else {
...
func (h *AcademicYearHandler) DeactivateAcademicYear(c *gin.Context) {
	...
	if err := h.service.DeactivateAcademicYear(uint(id)); err != nil {
		c.JSON(http.StatusInternalServerError, gin.H{"error": err.Error()})
		return
	}
...
func (h *AcademicYearHandler) GetActiveAcademicYear(c *gin.Context) {
	academicYear, err := h.service.GetActiveAcademicYear()
	if err != nil {
